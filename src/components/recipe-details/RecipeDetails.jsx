...
export default function RecipeDetails() {
  ...
  const { title, img, desc, cat, recipe, videoLink } = recipeItem;
    ...
    <div className="min-vh-100 d-flex flex-column bg-body-tertiary recipe-page">
      ...
      <style>{`
        @media print {
          ...
          /* Предотвратява срязване на елементи на две страници */
          .print-no-break {
            page-break-inside: avoid;
          }
          /* Добавя лека рамка на хартия за по-добра четимост */
          .print-border {
          ...
          /* На хартия искаме съставките да заемат по-малко място */
          .print-ing-box {
        ...
        <div className="card shadow-lg border-0 rounded-4 overflow-hidden bg-body print-border mb-5">
          ...
          <div className="row g-0 px-4 px-md-5 pb-5">
            ...
            <div className="col-lg-4 pe-lg-5 mb-5 mb-lg-0 print-no-break">
              <div className="bg-body-tertiary p-4 rounded-4 h-100 border print-ing-box shadow-sm">
              ...
              <div className="d-flex flex-column gap-4 mt-2">
                {recipe.steps.map((step, idx) => (
                  <div key={idx} className="d-flex align-items-start gap-3 print-no-break">
                    <span 
                      className="badge bg-warning text-dark rounded-circle d-flex align-items-center justify-content-center fs-5 flex-shrink-0 shadow-sm"
                      style={{ width: "40px", height: "40px" }}
                    >
                      {idx + 1}
                    </span>
                    <p className="mb-0 text-body pt-1" style={{ fontSize: "1.1rem", lineHeight: "1.7" }}>
                      {step}
                    </p>
                  </div>
                ))}
